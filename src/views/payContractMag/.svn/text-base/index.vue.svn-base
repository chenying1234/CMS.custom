...
export default {
  ...
  methods: {
    ...
    addNewContract() {
      this.$router.push({name: '支出合同添加'});
    },
    editContract(row) {
      this.$router.push({name: '支出合同修改', params: { id: row.Id}});
    },
    deleteContract(row) {
    ...
    showDetailContract(row) {
      this.$router.push({name: '支出合同详情', params: { pid: row.Id}});
    },
    keepFile(row) {
      this.$router.push({name: '支出合同存档', params: { pid: row.Id}});
    }
  }
}
</script>
